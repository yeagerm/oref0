...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    for (var i=ZTpredBGs.length-1; i > 12; i--) {
    ...
    // if we have UAM but no COB, average IOB, ZT, and UAM
    ...
    if ( minZTGuardBG > avgPredBG ) {
        avgPredBG = ( avgPredBG + minZTGuardBG ) / 2;
    }
    ...
    if (meal_data.carbs) {
        ...
        */
        ...
        if ( minZTGuardBG < threshold || minZTGuardBG > minUAMPredBG ) {
    ...
    if ( glucose_status.delta > 0.15 * bg ) {
        console.error("Delta",glucose_status.delta,"> 15% of BG",bg,"- disabling SMB");
        rT.reason += "Delta "+glucose_status.delta+" > 15% of BG "+bg+": SMB disabled; ";
        enableSMB = false;
    }
